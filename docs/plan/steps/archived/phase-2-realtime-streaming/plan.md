...
- This step's own `Validation` below is unit-only — per root `AGENTS.md`'s "Validation Conventions"
  rule (added by step `00603` after this gap went undetected for four subsequent steps), any future
  change to this composition logic must also include a real, executed end-to-end prompt check in
  its own `Validation` → `Commands`, not deferred to Step 6's final E2E gate.
